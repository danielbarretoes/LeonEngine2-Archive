...
#include "Lightmass/ULightBuildSettings.hpp"
...
namespace Leon {
    ...
    TEST_SUITE("PlayerSpawnTests") {
        TEST_CASE("match spawn possesses a character on a PlayerStart") {
            ...
            REQUIRE(pc);
            auto* ch = pc->GetPawn<AShooterCharacter>();
            REQUIRE(ch);
    ...
    TEST_SUITE("PlayerPossessionTests") {
        TEST_CASE("controller remains after pawn freeze on death path") {
            FMatchWorld f;
            auto* pc = f.World->GetFirstPlayerController();
            auto* ch = f.World->SpawnActor<AShooterCharacter>("Pawn");
            pc->Possess(ch);
            CHECK(pc->GetPawn() == ch);
    ...
    TEST_SUITE("HitMarkerTests") {
        TEST_CASE("confirmed hit arms the player-controller marker") {
            FMatchWorld f;
            auto* pc = dynamic_cast<AShooterPlayerController*>(f.World->GetFirstPlayerController());
            REQUIRE(pc);
    ...
    TEST_SUITE("ShootingIntegrationTests") {
        TEST_CASE("server trace applies damage and hit confirm") {
            ...
            auto* attacker = f.World->SpawnActor<AShooterCharacter>("Atk");
            auto* victim = f.World->SpawnActor<AShooterCharacter>("Vic");
            auto* pca = dynamic_cast<AShooterPlayerController*>(f.World->GetFirstPlayerController());
            REQUIRE(pca);
            auto* psv = f.World->SpawnActor<AShooterPlayerState>("PSV");
            if (auto* psa = dynamic_cast<AShooterPlayerState*>(pca->GetPlayerState()))
    ...
    TEST_SUITE("GameStateReplicationTests") {
        TEST_CASE("loopback replicates match state") {
            ...
            gm->HUDClass = "None";
            server->BeginPlay();
            gm->GetShooterGameState()->SetMatchState(EShooterMatchState::Playing);
            gm->GetShooterGameState()->SetTeam1Kills(4);
            for (int i = 0; i < 3; ++i) {
                server->Tick(FTimestep(0.05f));
                client->Tick(FTimestep(0.05f));
            }
            auto* gs = dynamic_cast<AShooterGameState*>(client->GetGameState());
            REQUIRE(gs);
            CHECK(gs->GetMatchState() == EShooterMatchState::Playing);
    ...
    TEST_SUITE("PlayerStatePersistenceTests") {
        TEST_CASE("kills survive pawn respawn") {
            ...
            REQUIRE(ps);
            ps->AddKill();
            auto* pawn = f.World->SpawnActor<AShooterCharacter>("P");
            pc->Possess(pawn);
            CHECK(ps->GetKills() == 1);
            CHECK(pawn->GetShooterPlayerState() == ps);
        }
    }
    ...
    TEST_SUITE("LightBakeIntegrationTests") {
        TEST_CASE("medium bake preset is deterministic") {
            auto a = ULightBuildSettings::Medium().ToLightmass();
            auto b = ULightBuildSettings::Medium().ToLightmass();
