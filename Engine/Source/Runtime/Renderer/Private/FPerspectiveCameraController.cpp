#include "Renderer/FPerspectiveCameraController.hpp"
...
namespace Leon {
    ...
    FPerspectiveCameraController::FPerspectiveCameraController(float InFOV, float InAspectRatio, float InNearClip,
                                                               float InFarClip)
        : Camera(InFOV, InAspectRatio, InNearClip, InFarClip) {}

    void FPerspectiveCameraController::OnUpdate(FTimestep InTs) {
        float speed = TranslationSpeed * InTs.GetSeconds();
        ...
        if (FInput::IsMouseButtonPressed(Mouse::ButtonRight) || FInput::IsMouseButtonPressed(Mouse::ButtonLeft)) {
            ...
            float xOffset = (mouseX - LastMousePosition.x) * RotationSpeed;
            float yOffset = (LastMousePosition.y - mouseY) * RotationSpeed;
        ...
        if (FInput::IsGamepadConnected(0)) {
            ...
            auto [leftX, leftY] = FInput::GetGamepadLeftStick(0, GamepadDeadzone);
            ...
            auto [rightX, rightY] = FInput::GetGamepadRightStick(0, GamepadDeadzone);
            if (std::abs(rightX) > 0.0f || std::abs(rightY) > 0.0f) {
                float yawOffset = rightX * GamepadRotationSpeed * InTs.GetSeconds();
                float pitchOffset = -rightY * GamepadRotationSpeed * InTs.GetSeconds();

                Camera.SetRotation(Camera.GetPitch() + pitchOffset, Camera.GetYaw() + yawOffset);
            }
        }
    ...
    bool FPerspectiveCameraController::OnWindowResized(FWindowResizeEvent& InEvent) {
        Camera.SetViewportSize(InEvent.GetWidth(), InEvent.GetHeight());
        return false;
    }
