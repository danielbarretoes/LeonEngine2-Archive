...
namespace Leon {
    ...
    bool FConfigFile::Load(const std::string& InFilePath) {
        ...
        LE_CORE_INFO("FConfigFile: Successfully loaded '{0}' ({1} sections)", InFilePath, m_Data.size());
        return true;
    }

    bool FConfigFile::Save(const std::string& InFilePath) {
        std::string path = InFilePath.empty() ? m_FilePath : InFilePath;
        if (path.empty()) {
    ...
    void FConfigFile::SetBool(const std::string& InSection, const std::string& InKey, bool InValue) {
        m_Data[InSection][InKey] = InValue ? "True" : "False";
    }
