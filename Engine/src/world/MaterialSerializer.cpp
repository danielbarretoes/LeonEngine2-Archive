...
namespace Leon {
    ...
    bool FMaterialSerializer::SerializeText(std::string& OutText, const FMaterial& InMaterial) {
        ...
        std::string albedoPath = InMaterial.GetAlbedoMap() ? InMaterial.GetAlbedoMap()->GetPath() : InMaterial.GetTexturePath(0);
        if (!albedoPath.empty())
            ss << "  AlbedoMap: \"" << albedoPath << "\"\n";

        std::string normalPath = InMaterial.GetNormalMap() ? InMaterial.GetNormalMap()->GetPath() : InMaterial.GetTexturePath(1);
        if (!normalPath.empty())
            ss << "  NormalMap: \"" << normalPath << "\"\n";

        std::string metallicPath = InMaterial.GetMetallicMap() ? InMaterial.GetMetallicMap()->GetPath() : InMaterial.GetTexturePath(2);
        if (!metallicPath.empty())
            ss << "  MetallicMap: \"" << metallicPath << "\"\n";

        std::string roughnessPath = InMaterial.GetRoughnessMap() ? InMaterial.GetRoughnessMap()->GetPath() : InMaterial.GetTexturePath(3);
        ...
        std::string emissivePath = InMaterial.GetEmissiveMap() ? InMaterial.GetEmissiveMap()->GetPath() : InMaterial.GetTexturePath(5);
