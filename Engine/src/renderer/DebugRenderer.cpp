...
namespace Leon {
    ...
    TRef<FShader> FDebugRenderer::s_Shader = nullptr;
    TRef<FVertexArray> FDebugRenderer::s_VertexArray = nullptr;
    TRef<FVertexBuffer> FDebugRenderer::s_VertexBuffer = nullptr;
    std::vector<FDebugRenderer::FDebugVertex> FDebugRenderer::s_LineVertices;
    glm::mat4 FDebugRenderer::s_ViewProjection = glm::mat4(1.0f);

    void FDebugRenderer::Init() {
        LE_CORE_INFO("Initializing DebugRenderer Subsystem...");

        s_Shader = FShader::Create("Engine/Assets/Shaders/DebugLine.glsl");

        s_VertexArray = FVertexArray::Create();
        s_VertexBuffer = FVertexBuffer::Create(MaxLineVertices * sizeof(FDebugVertex));
        s_VertexBuffer->SetLayout({{EShaderDataType::Float3, "aPos"}, {EShaderDataType::Float4, "aColor"}});
        s_VertexArray->AddVertexBuffer(s_VertexBuffer);

        s_LineVertices.reserve(4096);
    }
    ...
    void FDebugRenderer::Flush() {
        ...
        s_VertexBuffer->SetData(s_LineVertices.data(),
                                static_cast<unsigned int>(s_LineVertices.size() * sizeof(FDebugVertex)));
    ...
    void FDebugRenderer::DrawLine(const glm::vec3& InP0, const glm::vec3& InP1, const glm::vec4& InColor) {
        if (s_LineVertices.size() + 2 >= MaxLineVertices)
            Flush();

        s_LineVertices.push_back({InP0, InColor});
        s_LineVertices.push_back({InP1, InColor});
    }
